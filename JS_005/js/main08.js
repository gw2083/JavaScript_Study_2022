...
btnOk?.addEventListener("click", () => {
  ...
  // input box 의 0 번 tag(첫번째 tag) 의 name 변수 이름을 가져오기
  const txtName = inputs[0].name;
  console.log(txtName);
  console.log(index[txtName]);

  // input box 의 name 변수 이름으로 index 객체로부터 위치값을 찾고 그 위치값으로 각각 요소 매칭
  spans[index[txtName]].innerText = inputs[index[txtName]].value;

  // for() 반복문 이용 : input 전체 내용을 span 에 모두 보여주기
  // input tag 의 개수와 span tag 의 개수가 같을 때
  // JS 에서만 사용가능
