...
/**
 * form 에서 POST method 로 전송된 데이터를 req.body 속성으로 접근하기 위한 설정
 */
 ...
 * get method 로 데이터가 전달되면 주소표시줄에 데이터가 노출되어 보안에 매우 취약하다
 * 또한, 전체 문자열의 길이가 255자가 넘으면 데이터가 잘린다
 * 암호화 등을 전혀 수행할 수 없다
 *
...
/**
 * form 에서 입력한 데이터를 POST method 로 전달받기
 * form 에서 입력한 데이터를 post 로 보내면 form package 의 body 영역에 데이터가 포장되어서 전송된다
 * 이 데이터는 일단 주소표시줄에 노출되지 않고 감춰져서 보안이 유리하다
 * 또한, body 영역에 포장된 데이터는 암호화가 비교적 쉽다
 * 따라서 암호화 도구(https)를 사용하면 데이터를 중간에 탈취하기가 어려워진다
