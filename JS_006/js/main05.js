...
const addrInput = () => {
  ...
  // 생성된 addr 객체를 addrList 배열에 추가하기
  addrList.push(addr);
  console.table(addrList);
  // localStorage : web browser 에 기본적으로 있는 데이터베이스
  // 소규모 이지만 데이터를 저장하는 공간(데이터 서버)을 제공해 준다
  // localStorage 에 myAddr 이라는 이름으로 나의 주소록(addrList) 배열에 저장된 데이터를 보관하겠다

  // localStorage 에는 String(text) type 의 데이터만 저장할 수 있다
  // 우리가 만든 addrList 는 객체를 배열로 만든 데이터
  // 이 데이터를 localStorage 에 보관할 수 없다
  // 그래서 JSON style 의 문자열로 변환하여 보관하기 JSON.stringify() 함수를 사용한다

  // JSON.stringify() 함수는 JS 에서 사용하는 어떤 데이터든지 문자열로 변화할 수 있는 기능을 갖는다
...
btnAdd?.addEventListener("click", () => {
  // addrCheck 함수에서 모든 유효성 검사가 완료되면 return true 를 하기 때문에
  // if() 비교하여 addrCheck() 실행결과가 true 이면...
  if (addrCheck()) {
    addrInput();
  }
});
