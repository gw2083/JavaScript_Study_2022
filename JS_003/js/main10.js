...
/**
 * 현재 문서에 클래스가 num1(.num1)인 친구들 중에 span tag 들만 가져와서 num1s 배열에 보관하라
 */
const num1s = document.querySelectorAll("span.num1");

/** 현재 문서에 클래스가 num2(.num2)인 친구들 중에 span tag 들만 가져와서 num2s 배열에 보관하라
 */
...
/**
 * 본문에 button tag 가 한개만 있기 때문에 별도의 id 부착하지 않고 직접 tag 를 가져와서 변수에 보관
 */
...
btn.addEventListener("click", () => {
  ...
  // num1s 배열과 num2s 배열의 개수가 다를 수 있다는 전제에서
  for (let i = 0; i < num1s.length; i++) {
    num1s.innerText = intNum1;
  }
  for (let i = 0; i < num2s.length; i++) {
    num2s.innerText = intNum2;
  }
  ...
  //result box 의 display style 속성을 block 하여 다시 보이도록 하여라
