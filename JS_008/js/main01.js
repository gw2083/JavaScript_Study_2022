...
document.addEventListener("DOMContentLoaded", () => {
  // 화면의 tag 요소를 js 코드에서 사용하기 위하여 tag 요소를 선택하여(select) 객체에 보관하기
  // inputs, inputScores 는 SelectAll() 을 사용하여 다수의 요소를 보관하기 위한 객체 배열 (for()사용가능)
   ...
   *      학번, 이름 : 입력이 되지 않았을 때 alert 띄우고 다시 입력
   *      국어, 영어, 수학 : 입력이 되지 않았을 때 alert, 다시 입력
   *      점수범위(0~100)에서 벗어나면 : alert, 다시 입력
   ...
   * 6. 다시(새로고침) 시작했을 때 localStorage 에 보관된 데이터를 읽어와서 table 에 보이기
   */
  ...
  const scoreCheck = () => {
    ...
    for (let i = 0; i < inputScores.length; i++) {
      ...
      /**
       * for() 반복문에 의해서 점수를 입력받는 input 만큼 코드가 반복실행된다 (여기에서는 3번 실행)
       * 만약 첫번째 input 값을 검사하는데 1번 조건문이 실행된다 = 아무것도 입력되지 않았을 때
       *  -> "점수를 입력..." alert 이 뜰 것이고, 실행은 input.focus() 로 점프한다
       * 1번 조건문이 실행되지 않았으면 2번 조건문 검사를 하고, 2번 조건문이 실행된다면 = 점수가 0보다 작고 100보다 큰 경우
       *  -> "점수 범위..." alert 이 뜰 것이고, 실행은 input.focus() 로 점프한다
       * 1번, 2번 조건문이 모두 통과되면 = 점수가 정상으로 입력
       *  -> else 에 있는 continue 가 실행되고, 다음 점수를 검사를 시도한다
       * 만약 1번, 2번 검사에서 한번이라도 문제를 찾으면
       *  -> input.focus(), return false 가 실행되어 검사과정을 즉시 중단한다. 그리고 return false 로 false 값을 되돌려준다
       */
      if (!input.value) {
        // 1
      ...
      // 입력된 text 에 선택block 을 설정하여 즉시 문자열을 입력하면 text 를 지우고 다시 입력, 방향키를 누르면 문자열 중간의 글자들을 수정할 수 있도록 하는 역할
      input.select();
      input.focus();
      return false;
    } // for() end
   ...
   * 1. 입력할 성적데이터는 항목이 5개 이다
   * 2. 다수의 학생 성적들을 입력해야 할 것이다
   */
  const scoreInput = () => {
    // 입력된 성적을 성적들(scores) 배열에 추가
    /**
     * 한 사람의 성적에는 5개의 항목(학번, 이름, 국어, 영어, 수학)
     * 5개의 항목이 하나의 묶음으로 구성되어야 할 것이기 때문에 이 데이터를 객체로 묶어줘야 한다
    ...
    // 평균 계산
    // 평균은 반드시 전체 총 합의 계산이 완료된 후 실행
    // 총점을 계산하는 for() 반복문 내에서 계산하면 오차 발생 확률 높아진다
    ...
    // scoreInput() 함수가 끝나는 부분 : 여기에 다다르면 score 는 소멸, scores 는 score들을 담고 계속 유지
  };
  ...
  const scorePrint = () => {
    ...
    // 전체 데이터(socre 들) 개수만큼 반복하면서 TR 생성하기
  ...
  btnOk?.addEventListener("click", () => {
    /**
     * scoreCheck() 함수는 유효성 검사가 모두 정상으로 완료되면 true 를 return 하고, 그렇지 않으면 false 를 return 할 것이다 라는 것을 미리예상하고 작성하는 코드
     */
    if (scoreCheck()) {
      scoreInput();
      scoreSave();
